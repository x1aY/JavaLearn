...
public class AD_midArray_4 {

    public static void main(String[] args) {
        AD_midArray_4 solution = new AD_midArray_4();
        int[] fst = { 1, 3, 4, 7 }, sec = { 2, 5, 6 };
        solution.findMedianSortedArrays(fst, sec);
    }

    public double findMedianSortedArrays(int[] fst, int[] sec) {
        double mid = 0;
        return mid;
    }
    ...
    class solution {
        ...
        private int getKth(int[] nums1, int start1, int end1, int[] nums2, int start2, int end2, int k) {
            ...
            // 因为k代表个数，而不是索引，那么从nums2后再找k个数，那个就是start2 +
            // k-1索引处就行了。因为还包含nums2[start2]也是一个数。因为它在上次迭代时并没有被排除
            if (len1 == 0)
